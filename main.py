...
from schemas import NewTrip, CreatedTrip, DeletedTrip
...
@app.post('/api/trips/create/', status_code=status.HTTP_201_CREATED, tags=['API', 'Trips'])
def create_trip(new_trip: NewTrip) -> CreatedTrip:
    created_trip = dao.create_trip(**new_trip.dict())
    return created_trip
...
def update_trip(
    ...
    trip = dao.update_trip(trip_id, updated_trip.dict())
    return trip
...
def index_web(request: Request):
    ...
    return templates.TemplateResponse('index.html', {'request': request, '': {}, 'title': 'Main page'})
